...
aws ecs update-service \
    --cluster file-service-production-cluster \
    --service file-service-production-service \
...
aws ecs list-tasks \
    --cluster file-service-production-cluster \
    --service-name file-service-production-service \
    --region us-east-1
```
...
2. Inspect application logs: `aws logs tail /ecs/file-service-production --follow`
...
aws rds create-db-snapshot \
    --db-instance-identifier file-service-production-db \
...
```bash
aws logs tail /ecs/file-service-production --follow --since 5m
```
...
aws logs filter-log-events \
    --log-group-name /ecs/file-service-production \
...
```bash
aws s3 ls s3://file-service-logs-production-<ACCOUNT_ID>/alb-access-logs/ --recursive
aws s3 cp s3://file-service-logs-production-<ACCOUNT_ID>/alb-access-logs/<path>/<file>.gz - | gzip -d | head -50
```
...
aws application-autoscaling describe-scaling-activities \
    ...
    --resource-id service/file-service-production-cluster/file-service-production-service \
    --region us-east-1
```
...
| 10 | DNS resolves (if domain) | `nslookup api.example.com` | Points to ALB |
