# 📖 Operational Runbook — Multi-Tenant File Service API
...
```
ECS Cluster:   file-service-dev-cluster
ECS Service:   file-service-dev-service
ALB:           file-service-dev-alb
RDS Instance:  file-service-dev-db
ECR Repo:      file-service-api-dev
S3 Bucket:     file-service-storage-dev-<account-id>
Secret:        file-service/dev/app-secrets
```
...
aws ecs describe-services \
    --cluster file-service-dev-cluster \
    --services file-service-dev-service \
...
aws ecs list-tasks \
    --cluster file-service-dev-cluster \
    --service-name file-service-dev-service \
    --output table
```
...
aws rds describe-db-instances \
    --db-instance-identifier file-service-dev-db \
...
aws ecs update-service \
    --cluster file-service-dev-cluster \
    --service file-service-dev-service \
...
aws ecs describe-task-definition \
    --task-definition file-service-dev-task \
...
aws logs get-log-events \
    --log-group-name "/ecs/file-service-dev" \
...
1. Navigate to **AWS Console** $\rightarrow$ **Elastic Container Service (ECS)** $\rightarrow$ **Clusters** $\rightarrow$ `file-service-dev-cluster`.
2. Under the **Tasks** tab, click **Run new task**.
3. Select **Fargate** launch type and task definition `file-service-dev-task`.
...
aws rds restore-db-instance-to-point-in-time \
    --source-db-instance-identifier file-service-dev-db \
    --target-db-instance-identifier file-service-dev-db-restored \
...
aws s3 ls s3://file-service-storage-dev-<ACCOUNT_ID>/ --recursive --summarize

# List files for a specific tenant
aws s3 ls s3://file-service-storage-dev-<ACCOUNT_ID>/main_app/
```
...
aws s3api list-object-versions \
    --bucket file-service-storage-dev-<ACCOUNT_ID> \
    --prefix "main_app/" \
    --max-items 20
...
aws cloudwatch get-metric-statistics \
    ...
    --dimensions Name=BucketName,Value=file-service-storage-dev-<ACCOUNT_ID> Name=StorageType,Value=StandardStorage \
...
aws secretsmanager describe-secret \
    --secret-id "file-service/dev/app-secrets" \
    --output table
```
...
aws ecs list-task-definitions \
    --family-prefix file-service-dev-task \
...
aws ecs update-service \
    --cluster file-service-dev-cluster \
    --service file-service-dev-service \
    --task-definition file-service-dev-task:<PREVIOUS_REVISION>
```
...
aws cloudwatch describe-alarms \
    --alarm-name-prefix "file-service-dev" \
