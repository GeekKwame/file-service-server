# 🚀 Deployment Guide — File Service Infrastructure & Application
...
```bash
git clone https://github.com/GeekKwame/file-service-server.git
...
```
alb_dns_name       = "file-service-dev-alb-XXXXXXXXX.us-east-1.elb.amazonaws.com"
api_url            = "http://file-service-dev-alb-XXXXXXXXX.us-east-1.elb.amazonaws.com"
ecr_repository_url = "123456789012.dkr.ecr.us-east-1.amazonaws.com/file-service-api-dev"
rds_endpoint       = "file-service-dev-db.xxxxx.us-east-1.rds.amazonaws.com:5432"
s3_bucket_name     = "file-service-storage-dev-123456789012"
```
...
```bash
docker build -t file-service-api .
```
...
```bash
# For dev environment:
docker tag file-service-api:latest <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/file-service-api-dev:latest

# For production:
docker tag file-service-api:latest <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/file-service-api-production:latest
```

### Step 4: Push Image to ECR

```bash
docker push <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/file-service-api-dev:latest
```
...
DNS for `example.com` is managed outside this AWS account. The ALB CNAME and the ACM validation CNAME are two different records.
...
```
fileservice.example.com  CNAME  file-service-dev-alb-256939318.us-east-1.elb.amazonaws.com
```
...
API URL: `https://fileservice.example.com/api/v1`
...
1. Manually empty the S3 buckets first:
   ```bash
   aws s3 rm s3://file-service-storage-dev-ACCOUNT_ID --recursive
   aws s3 rm s3://file-service-logs-dev-ACCOUNT_ID --recursive
   ```
