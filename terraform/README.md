# 🏗️ Terraform Infrastructure — File Service

This directory contains the complete Infrastructure-as-Code (IaC) for the **Multi-Tenant File Service API** on AWS, built with **Terraform v1.8+**.
...
| `project_name` | string | Project name prefix | `"file-service"` |
...
| `domain_name` | string | Custom domain name | `"fileservice.example.com"` |
...
|:---|:---|:---|
| `alb_dns_name` | ALB DNS endpoint | `file-service-dev-alb-XXX.us-east-1.elb.amazonaws.com` |
| `api_url` | Full API URL | `https://fileservice.example.com` |
...
| `ecr_repository_url` | ECR Docker repository | `123456789012.dkr.ecr.us-east-1.amazonaws.com/file-service-api-dev` |
| `s3_bucket_name` | S3 storage bucket | `file-service-storage-dev-123456789012` |
| `rds_endpoint` | PostgreSQL endpoint | `file-service-dev-db.xxx.rds.amazonaws.com:5432` |
| `ecs_cluster_name` | ECS cluster name | `file-service-dev-cluster` |
| `ecs_service_name` | ECS service name | `file-service-dev-service` |
| `cloudwatch_dashboard_name` | Dashboard name | `file-service-dev-dashboard` |
